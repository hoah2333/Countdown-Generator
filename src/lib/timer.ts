export function timerFunc(paramsOri: string[]) {
  ...
  let params = { time: timeParam, type: typeParam, custom: [customParam[0], customParam[1]] };
  ...
  return { warning: warning, text: outputs.text, timer: outputs.timer, status: time > now ? "before" : "after" };
}
...
function outputTimer(
  ...
  if (time > now) {
    text =
      params.custom[0] != "" ? params.custom[0]
      : params.type == "delete" ? "该页面将在以下时间后被删除："
      : params.type == "ban" ? "该用户将在以下时间后解除封禁："
      : "该计时器将在以下时间后到期：";
    ...
    text =
      params.custom[1] != "" ? params.custom[1]
      : params.type == "delete" ? "该页面已在以下时间前可被删除："
      : params.type == "ban" ? "该用户已在以下时间前可被解除封禁："
      : "该计时器已在以下时间前到期：";
    timer =
      (remainDay == 0 ? "" : `${remainDay} 日 `) +
      (remainHour == 0 && remainDay == 0 ? "" : `${remainHour} 时 `) +
